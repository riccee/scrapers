...
async def search(domain):
    ...
    for i in response.data:
        if domain == i["domain"]:
            cleanedTotal = i["employees"]
            return json.loads(cleanedTotal)
    ...
    if cleanedTotal == {}:
        ...
        async with async_playwright() as playwright:
            ...
            for key, value in total.items():
                if key not in cleanedTotal:
                    cleanedTotal[key] = value

            #push to database
            data, count = supabase.table('Employees').insert({"domain": domain, "employees": cleanedTotal}).execute() 
            return cleanedTotal
